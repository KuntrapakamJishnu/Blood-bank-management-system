...
const DonorDirectory = () => {
  ...
  const fetchDonors = async () => {
    ...
    try {
      ...
      if (userLocation?.latitude && userLocation?.longitude) {
        queryParams.set("latitude", String(userLocation.latitude));
        queryParams.set("longitude", String(userLocation.longitude));
        queryParams.set("maxDistanceKm", String(radiusKm));
      }
  ...
  }, [filters, searchTerm, userLocation, radiusKm]);

  const enableLocationMatching = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        toast.success("Location enabled. Showing nearby donors.");
        setIsLocating(false);
      },
      () => {
  ...
  const clearLocationMatching = () => {
    setUserLocation(null);
    setFilters((prev) => ({
      ...prev,
      sortBy: prev.sortBy === "distance" ? "lastDonation" : prev.sortBy,
    }));
    toast.success("Nearby matching disabled");
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-red-100 p-6 mb-6">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 flex flex-col md:flex-row gap-3 items-start md:items-center">
            <button
              type="button"
              onClick={userLocation ? clearLocationMatching : enableLocationMatching}
              disabled={isLocating}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-50 text-red-700 hover:bg-red-100 transition-colors disabled:opacity-60"
            >
              <LocateFixed size={16} />
              {isLocating
                ? "Getting location..."
                : userLocation
                  ? "Disable Nearby Matching"
